...
CrosstoolsBuilder::Architecture.define :Aarch64 do
  ...
  define_macro "make" do |*args|
    ...
    rescue CrosstoolsBuilder::Command::NonZeroExitError
      shell do
        make *args
      end
    end
  ...
  build do
    ...
    part "glibc-step-2", chdir: "glibc" do
      path_prepend File.join(@toolsdir, "bin")
      shell do
        rm "-rf", "build-step-2"
        mkdir "build-step-2"
        chdir "build-step-2" do
          run "../configure",
              "--prefix=/usr",
              "--libdir=/lib",
              "--host=#{@triple}",
    ...
    part "binutils-native", chdir: "binutils" do
      path_prepend File.join(@toolsdir, "bin")
      shell do
        rm "-rf", "build-native"
        mkdir "build-native"
    ...
    part "test" do
      require 'tempfile'
      path_prepend File.join(@toolsdir, "bin")
      source = Tempfile.new(["test", ".c"])
      execut = Tempfile.new("test")
      source.print <<"EOF"
#include <stdio.h>
#include <omp.h>
